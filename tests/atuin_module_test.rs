...
"#;
    ...
    assert!(
        plan.atoms.len() >= 3,
        "Should have at least 3 atoms after flattening"
    );
}
...
"#;
    ...
    let discovered = discover_modules(temp_dir.path()).unwrap();
    let loaded = load_modules(discovered);
    let successful_modules: Vec<_> = loaded.iter().filter_map(|r| r.as_ref().ok()).collect();

    // When no tag filter is provided, all modules should be included
    assert_eq!(
        successful_modules.len(),
        1,
        "Module with no tags should be included when no filter is applied"
    );
}
...
"#;
    ...
    let dev_filtered: Vec<_> = successful_modules
        .iter()
    ...
    assert_eq!(
        dev_filtered.len(),
        2,
        "Should have 2 modules with 'development' tag"
    );

    // Filter by "desktop" tag
    let desktop_filtered: Vec<_> = successful_modules
        .iter()
        .filter(|m| m.definition.tags.contains(&"desktop".to_string()))
        .collect();
    assert_eq!(
        desktop_filtered.len(),
        1,
        "Should have 1 module with 'desktop' tag"
    );

    // Filter by non-existent tag
    let none_filtered: Vec<_> = successful_modules
        .iter()
    ...
    assert_eq!(
        none_filtered.len(),
        0,
        "Should have 0 modules with non-existent tag"
    );
}
...
"#;
    ...
    let selected: Vec<_> = successful_modules
        .iter()
