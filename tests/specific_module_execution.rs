...
"#;
    ...
    let discovered = discover_modules(temp_dir.path()).unwrap();
    let loaded = load_modules(discovered);
    let all_modules: Vec<_> = loaded.into_iter()
        .filter_map(|r| r.ok())
        .collect();
    
    // Simulate module filtering by name (like --module flag)
    let selected_modules: Vec<_> = all_modules.into_iter()
...
"#;
    ...
    for module in &successful_modules {
        ...
        match module.definition.name.as_str() {
            ...
            "dependent" => {
                assert_eq!(plan.action_count, 1, "Dependent module should have 1 action");
                assert!(plan.atoms.len() >= 1, "Dependent module should have at least 1 atom");
            }
