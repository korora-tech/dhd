use crate::{
    atom::Atom,
    actions::package_install_v2::PackageManagerType,
};
use std::any::Any;
...
impl Atom for InstallPackagesAtom {
    ...
    fn execute(&self) -> anyhow::Result<()> {
        let (cmd, args) = match &self.manager {
            ...
            PackageManagerType::Dnf => {
                ("sudo", vec!["dnf", "install", "-y"])
            }
            PackageManagerType::Flatpak => {
                ("flatpak", vec!["install", "-y"])
            }
            ...
            PackageManagerType::Pacman => {
                ("sudo", vec!["pacman", "-S", "--noconfirm"])
            }
            PackageManagerType::Paru => {
                ("paru", vec!["-S", "--noconfirm"])
            }
            PackageManagerType::Pip => {
                ("pip", vec!["install"])
            }
            PackageManagerType::Snap => {
                ("sudo", vec!["snap", "install"])
            }
            PackageManagerType::Winget => {
                ("winget", vec!["install", "-e", "--id"])
            }
            PackageManagerType::Yum => {
                ("sudo", vec!["yum", "install", "-y"])
            }
            PackageManagerType::Auto => {
                return Err(anyhow::anyhow!("Auto package manager should have been resolved"));
            }
        };
    ...
    fn describe(&self) -> String {
        format!("Install {} packages using {:?}", self.packages.len(), self.manager)
    }
